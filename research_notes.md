...
## Section 6: ZeRO1 Optimizer Sharding

**Setup (`cs336_systems/assignment_6_zero1.py`):** `ZeRO1` wraps an arbitrary `optimizer_cls` (here, the assignment's own bf16 `AdamW`) and shards parameters round-robin across ranks (`owner = param_counter % world_size`) — each rank only constructs a local optimizer over its owned shard, so optimizer state (`m`, `v`) is only ever materialized for that shard. `step()` runs the local optimizer step, then every rank `dist.broadcast`s every parameter from its owner so all ranks end up with the fully updated, unsharded model. `WORLD_SIZE=2`, NCCL, bf16, `MODEL_CONFIG_S`, `BATCH_SIZE=16`. Memory is measured rank 0 only (after `torch.cuda.set_device(device)` fix — see below), via `memory_allocated()` deltas around each phase plus `reset_peak_memory_stats()`/`max_memory_allocated()` around the optimizer step.

### Measured: plain `AdamW` (no sharding) vs. `ZeRO1`

| Phase | Plain AdamW | ZeRO1 (2 ranks) |
|---|---|---|
| Model static mem | 256.10 MB | 256.10 MB |
| Input tensor mem | 64.00 KB | 64.00 KB |
| Peak mem, FWD+BWD | 7886.59 MB | 7886.59 MB |
| Peak mem, optimizer step | 1264.08 MB | 1045.05 MB |
| Optimizer state mem (steady-state delta) | 501.95 MB | 282.91 MB |

**Do the results align with expectations? Yes, with one caveat.** Without sharding, each rank holds the optimizer state (`m`, `v`) for **all** parameters: since both are `torch.zeros_like(grad)` in bf16 (same dtype/size as the model), steady-state state memory is ≈2× model size (501.95 MB ≈ 2 × 256.10 MB, matching the analysis in the conversation above). With `ZeRO1` and `world_size=2`, each rank's local optimizer only owns ~half the parameters (round-robin sharding), so it should only allocate `m`/`v` for that half — naively, ≈501.95 / 2 ≈ 251 MB. The measured 282.91 MB is somewhat higher than that 2-shard estimate, which makes sense: `param_counter % world_size` shards at the *parameter-tensor* granularity (e.g. whole weight matrices), not by raw byte count, so an uneven split across rank 0 vs. rank 1 (e.g. rank 0 happening to own a couple of the larger weight matrices) plus the same non-in-place-temporary effects discussed earlier (`grad**2`, `sqrt(v_t)`, fresh `m_t`/`v_t`) account for the gap. FWD+BWD peak is identical in both rows (7886.59 MB) because activation memory and gradient sync don't change with ZeRO1 — sharding only affects the optimizer's persistent state, not the forward/backward graph. The reduction does show up exactly where expected: peak optimizer-step memory drops from 1264.08 MB → 1045.05 MB (~17% lower), and steady-state optimizer state drops from 501.95 MB → 282.91 MB (~44% lower) — directly reflecting that each rank now only carries optimizer state for its own shard instead of the full model.
